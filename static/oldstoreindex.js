    ...
    mutations: {
      ...
      */
      ...
      sumatoriaPeliculas(state, datos) {
        datos.map(
          v =>
            (v.sumvisitas = v.visitas.reduce(
              (prev, cur) => prev + cur.visitas,
              0
            ))
        );
        ...
        datos.map(datos => {
          for (let index = 0; index < datos.temporadas.length; ++index) {
            var esactiva = "NO";
            var dias = 0;
            if (datos.temporadas[index].salida == null) {
              esactiva = "SI";
            } else {
              dias =
                datos.temporadas[index].salida -
                datos.temporadas[index].entrada;
            }
          }
      ...
      fechasalidacambiada(state, datofecha) {
        ...
        state.peliculas[peliculaAfectada].temporadas[temporadaafectada].salida =
          datofecha.salida;
      },
      ...
      visitasPeliculas(state) {
        ...
        for (let i = 0; i < datos.length; ++i) {
          for (let j = 0; j < datos[i].visitas.length; ++j) {
            let nombrepelicula = lodash.find(state.peliculas, [
              "id",
              datos[i].id
            ]).titulo;
